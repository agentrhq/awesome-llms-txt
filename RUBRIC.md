...
|------:|---|---|---|
| **20** | **Coverage** | Section count (cap 5 pts), total link count (log-scaled, cap 10 pts), canonical-section diversity (docs / api / quickstart / guides / auth / errors / changelog / pricing / webhooks / sdk — cap 5 pts) | Heaviest weight. An incomplete index is useless even when perfectly formatted. |
...
- **"Linked-content stability shouldn't be sampled — check them all."** Cost / signal tradeoff. Sampling 8 catches 95% of regressions and respects rate limits on monthly crawls. Open to revisiting if better data appears.
