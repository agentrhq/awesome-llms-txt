---
domain: icorner.net
display_name: Icorner
category: dev-tools
score: 37
grade: D
last_scored: 2026-05-27
verified_url: https://icorner.net/llms.txt
file_size_bytes: 2411
link_count: 0
value_link_count: 0
section_count: 0
freshness_age_days: null
---

# Icorner

![Score 37/100 — Grade D](../../docs/badge/icorner.net.svg)

Category: **dev-tools** · [Live llms.txt](https://icorner.net/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** iCorner - Your Online Corner, Where Your Web Begins


**File facts:** 2.4 KB · 0 `## sections` · 0 links (0 after low-value discount) · Freshness: unknown (no `Last-Modified` header).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 6/18 | _no_blockquote_summary, no_structure, malformed_link_items_ |
| Coverage | 0/20 | _thin_sectioning, few_value_links, missing_canonical_sections_ |
| Agent-action declarations | 0/14 | _no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 5/10 | _no_last_modified_header_ |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 6/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Discoverability (8/8)
- Size discipline (6/6)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Spec compliance (6/18): no_blockquote_summary, no_structure, malformed_link_items
- Coverage (0/20): thin_sectioning, few_value_links, missing_canonical_sections
- Agent-action declarations (0/14): no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 37 (D)](https://raw.githubusercontent.com/agentrhq/awesome-llms-txt/main/docs/badge/icorner.net.svg)](https://github.com/agentrhq/awesome-llms-txt/tree/main/sites/icorner.net)
```

## Reproduce this score

```bash
npx llms-txt-score https://icorner.net/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
