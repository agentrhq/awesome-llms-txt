---
domain: sketchready.com
display_name: Sketchready
category: dev-tools
score: 24
grade: F
last_scored: 2026-05-27
verified_url: https://sketchready.com/llms.txt
file_size_bytes: 68
link_count: 0
value_link_count: 0
section_count: 0
freshness_age_days: null
---

# Sketchready

![Score 24/100 — Grade F](../../docs/badge/sketchready.com.svg)

Category: **dev-tools** · [Live llms.txt](https://sketchready.com/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** _missing_


**File facts:** 0.1 KB · 0 `## sections` · 0 links (0 after low-value discount) · Freshness: unknown (no `Last-Modified` header).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 0/18 | _no_h1, no_blockquote_summary, no_structure_ |
| Coverage | 0/20 | _thin_sectioning, few_value_links, missing_canonical_sections_ |
| Agent-action declarations | 0/14 | _no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 5/10 | _no_last_modified_header_ |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 0/6 | _empty_or_stub_ |
| Content-Type & encoding | 3/4 | _no_utf8_charset_ |
| Voice | 2/2 | |

## What's exceptional

- Discoverability (8/8)
- Voice (2/2)

## What's weak

- Spec compliance (0/18): no_h1, no_blockquote_summary, no_structure
- Coverage (0/20): thin_sectioning, few_value_links, missing_canonical_sections
- Agent-action declarations (0/14): no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec
- Auth signposting (0/8): no_auth_keywords, no_auth_section
- Size discipline (0/6): empty_or_stub

## Embed the badge

```markdown
[![llms.txt score 24 (F)](https://raw.githubusercontent.com/agentrhq/awesome-llms-txt/main/docs/badge/sketchready.com.svg)](https://github.com/agentrhq/awesome-llms-txt/tree/main/sites/sketchready.com)
```

## Reproduce this score

```bash
npx llms-txt-score https://sketchready.com/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
