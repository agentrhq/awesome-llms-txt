---
domain: rsbuild.dev
display_name: Rsbuild
category: dev-tools
score: 65
grade: B
last_scored: 2026-05-27
verified_url: https://rsbuild.rs/llms.txt
file_size_bytes: 28706
link_count: 192
value_link_count: 182
section_count: 5
freshness_age_days: null
---

# Rsbuild

![Score 65/100 — Grade B](../../web/badge/rsbuild.dev.svg)

Category: **dev-tools** · [Live llms.txt](https://rsbuild.rs/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** Rsbuild

> Rsbuild is a high-performance build tool powered by Rspack.

**File facts:** 28.0 KB · 5 `## sections` · 192 links (182 after low-value discount) · Freshness: unknown (no `Last-Modified` header).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 18/18 | |
| Coverage | 14/20 | |
| Agent-action declarations | 5/14 | _no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 5/10 | _no_last_modified_header_ |
| Discoverability | 6/8 | _redirect_chain_ |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 5/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Spec compliance (18/18)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Agent-action declarations (5/14): no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 65 (B)](https://raw.githubusercontent.com/agentrhq/awesome-llms-txt/main/web/badge/rsbuild.dev.svg)](https://github.com/agentrhq/awesome-llms-txt/tree/main/sites/rsbuild.dev)
```

## Reproduce this score

```bash
npx llms-txt-score https://rsbuild.rs/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.

---

Curated by [Authsome](https://authsome.dev) · agent identity for third-party APIs.
