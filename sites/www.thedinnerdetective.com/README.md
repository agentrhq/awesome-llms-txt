---
domain: www.thedinnerdetective.com
display_name: Thedinnerdetective
category: dev-tools
score: 52
grade: C
last_scored: 2026-05-27
verified_url: https://www.thedinnerdetective.com/llms.txt
file_size_bytes: 1130
link_count: 5
value_link_count: 5
section_count: 5
freshness_age_days: 302
---

# Thedinnerdetective

![Score 52/100 — Grade C](../../web/badge/www.thedinnerdetective.com.svg)

Category: **dev-tools** · [Live llms.txt](https://www.thedinnerdetective.com/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** The Dinner Detective True Crime Murder Mystery Dinner Show

> North America's largest interactive comedy true crime murder mystery dinner show; immersive entertainment where you solve a crime over dinner.

**File facts:** 1.1 KB · 5 `## sections` · 5 links (5 after low-value discount) · Freshness: 302 days (per `Last-Modified`).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 18/18 | |
| Coverage | 6/20 | _few_value_links, missing_canonical_sections_ |
| Agent-action declarations | 0/14 | _no_llms_full_link, no_md_url_twins, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 3/10 | _stale_302_days_ |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 6/6 | |
| Content-Type & encoding | 3/4 | _no_utf8_charset_ |
| Voice | 2/2 | |

## What's exceptional

- Spec compliance (18/18)
- Discoverability (8/8)
- Size discipline (6/6)
- Voice (2/2)

## What's weak

- Coverage (6/20): few_value_links, missing_canonical_sections
- Agent-action declarations (0/14): no_llms_full_link, no_md_url_twins, no_agent_signposts
- Freshness (3/10): stale_302_days
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 52 (C)](https://raw.githubusercontent.com/agentrhq/awesome-llms-txt/main/web/badge/www.thedinnerdetective.com.svg)](https://github.com/agentrhq/awesome-llms-txt/tree/main/sites/www.thedinnerdetective.com)
```

## Reproduce this score

```bash
npx llms-txt-score https://www.thedinnerdetective.com/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
