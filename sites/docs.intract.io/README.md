---
domain: docs.intract.io
display_name: Community Guide
category: dev-tools
score: 61
grade: C
last_scored: 2026-05-27
verified_url: https://docs.intract.io/llms.txt
file_size_bytes: 8152
link_count: 63
value_link_count: 61
section_count: 3
freshness_age_days: null
---

# Community Guide

![Score 61/100 · Grade C](../../docs/badge/docs.intract.io.svg)

Category: **dev-tools** · [Live llms.txt](https://docs.intract.io/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** Community Guide


**File facts:** 8.0 KB · 3 `## sections` · 63 links (61 after low-value discount) · Freshness: unknown (no `Last-Modified` header).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 14/18 | _multi_h1_2, no_blockquote_summary_ |
| Coverage | 11/20 | |
| Agent-action declarations | 5/14 | _no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 5/10 | _no_last_modified_header_ |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 6/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Discoverability (8/8)
- Size discipline (6/6)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Agent-action declarations (5/14): no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 61 (C)](https://raw.githubusercontent.com/agentrhq/awesome-llms-txt/main/docs/badge/docs.intract.io.svg)](https://github.com/agentrhq/awesome-llms-txt/tree/main/sites/docs.intract.io)
```

## Reproduce this score

```bash
npx llms-txt-score https://docs.intract.io/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
