---
domain: docs.contentsquare.com
display_name: Contentsquare Tech Docs
category: observability
score: 74
grade: B
last_scored: 2026-05-27
verified_url: https://docs.contentsquare.com/llms.txt
file_size_bytes: 66106
link_count: 364
value_link_count: 349
section_count: 14
freshness_age_days: 6
---

# Contentsquare Tech Docs

![Score 74/100 — Grade B](../../web/badge/docs.contentsquare.com.svg)

Category: **observability** · [Live llms.txt](https://docs.contentsquare.com/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** Contentsquare Tech Docs

> Contentsquare is a unified experience analytics, product analytics, and voice of customer platform for web and mobile applications.

**File facts:** 64.6 KB · 14 `## sections` · 364 links (349 after low-value discount) · Freshness: 6 days (per `Last-Modified`).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 18/18 | |
| Coverage | 16/20 | _missing_canonical_sections_ |
| Agent-action declarations | 6/14 | _no_llms_full_link, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 10/10 | |
| Discoverability | 8/8 | |
| Auth signposting | 2/8 | _no_auth_section_ |
| Size discipline | 2/6 | _oversized_65kb_ |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Spec compliance (18/18)
- Freshness (10/10)
- Discoverability (8/8)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Auth signposting (2/8): no_auth_section
- Size discipline (2/6): oversized_65kb

## Embed the badge

```markdown
[![llms.txt score 74 (B)](https://raw.githubusercontent.com/agentrhq/awesome-llms-txt/main/web/badge/docs.contentsquare.com.svg)](https://github.com/agentrhq/awesome-llms-txt/tree/main/sites/docs.contentsquare.com)
```

## Reproduce this score

```bash
npx llms-txt-score https://docs.contentsquare.com/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
