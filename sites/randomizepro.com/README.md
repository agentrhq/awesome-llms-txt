---
domain: randomizepro.com
display_name: Randomizepro
category: dev-tools
score: 49
grade: D
last_scored: 2026-05-27
verified_url: https://randomizepro.com/llms.txt
file_size_bytes: 2106
link_count: 15
value_link_count: 14
section_count: 0
freshness_age_days: 0
---

# Randomizepro

![Score 49/100 — Grade D](../../docs/badge/randomizepro.com.svg)

Category: **dev-tools** · [Live llms.txt](https://randomizepro.com/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** https://randomizepro.com/ llms.txt


**File facts:** 2.1 KB · 0 `## sections` · 15 links (14 after low-value discount) · Freshness: 0 days (per `Last-Modified`).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 10/18 | _h1_is_url, no_blockquote_summary_ |
| Coverage | 3/20 | _missing_canonical_sections_ |
| Agent-action declarations | 0/14 | _no_llms_full_link, no_md_url_twins, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 10/10 | |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 6/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Freshness (10/10)
- Discoverability (8/8)
- Size discipline (6/6)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Coverage (3/20): missing_canonical_sections
- Agent-action declarations (0/14): no_llms_full_link, no_md_url_twins, no_agent_signposts
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 49 (D)](https://raw.githubusercontent.com/agentrhq/awesome-llms-txt/main/docs/badge/randomizepro.com.svg)](https://github.com/agentrhq/awesome-llms-txt/tree/main/sites/randomizepro.com)
```

## Reproduce this score

```bash
npx llms-txt-score https://randomizepro.com/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
