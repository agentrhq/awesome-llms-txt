---
domain: www.dbe.academy
display_name: Dbe
category: dev-tools
score: 43
grade: D
last_scored: 2026-05-27
verified_url: https://www.dbe.academy/llms.txt
file_size_bytes: 10063
link_count: 33
value_link_count: 32
section_count: 0
freshness_age_days: 343
---

# Dbe

![Score 43/100 — Grade D](../../web/badge/www.dbe.academy.svg)

Category: **dev-tools** · [Live llms.txt](https://www.dbe.academy/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** https://www.dbe.academy/ llms.txt


**File facts:** 9.8 KB · 0 `## sections` · 33 links (32 after low-value discount) · Freshness: 343 days (per `Last-Modified`).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 10/18 | _h1_is_url, no_blockquote_summary_ |
| Coverage | 5/20 | _missing_canonical_sections_ |
| Agent-action declarations | 0/14 | _no_llms_full_link, no_md_url_twins, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 3/10 | _stale_343_days_ |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 5/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Discoverability (8/8)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Coverage (5/20): missing_canonical_sections
- Agent-action declarations (0/14): no_llms_full_link, no_md_url_twins, no_agent_signposts
- Freshness (3/10): stale_343_days
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 43 (D)](https://raw.githubusercontent.com/agentrhq/awesome-llms-txt/main/web/badge/www.dbe.academy.svg)](https://github.com/agentrhq/awesome-llms-txt/tree/main/sites/www.dbe.academy)
```

## Reproduce this score

```bash
npx llms-txt-score https://www.dbe.academy/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.

---

Curated by [Authsome](https://authsome.dev) · agent identity for third-party APIs.
