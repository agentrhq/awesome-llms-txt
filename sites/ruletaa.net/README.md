---
domain: ruletaa.net
display_name: Ruletaa
category: dev-tools
score: 54
grade: C
last_scored: 2026-05-27
verified_url: https://ruletaa.net/llms.txt
file_size_bytes: 2842
link_count: 20
value_link_count: 11
section_count: 3
freshness_age_days: null
---

# Ruletaa

![Score 54/100 — Grade C](../../web/badge/ruletaa.net.svg)

Category: **dev-tools** · [Live llms.txt](https://ruletaa.net/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** Ruletaa - Spin the Random Wheel Online

> Ruletaa is a user-friendly web app that lets you create and spin customizable random wheels for decisions, games, and prize draws. Enjoy fair, instant, and private results on any device.

**File facts:** 2.8 KB · 3 `## sections` · 20 links (11 after low-value discount) · Freshness: unknown (no `Last-Modified` header).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 18/18 | |
| Coverage | 5/20 | _missing_canonical_sections_ |
| Agent-action declarations | 0/14 | _no_llms_full_link, no_md_url_twins, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 5/10 | _no_last_modified_header_ |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 6/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Spec compliance (18/18)
- Discoverability (8/8)
- Size discipline (6/6)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Coverage (5/20): missing_canonical_sections
- Agent-action declarations (0/14): no_llms_full_link, no_md_url_twins, no_agent_signposts
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 54 (C)](https://raw.githubusercontent.com/agentrhq/awesome-llms-txt/main/web/badge/ruletaa.net.svg)](https://github.com/agentrhq/awesome-llms-txt/tree/main/sites/ruletaa.net)
```

## Reproduce this score

```bash
npx llms-txt-score https://ruletaa.net/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
