---
domain: developer.tryfinch.com
display_name: Finch
category: data
score: 76
grade: B+
last_scored: 2026-05-27
verified_url: https://developer.tryfinch.com/llms.txt
file_size_bytes: 21939
link_count: 110
value_link_count: 110
section_count: 3
freshness_age_days: 0
---

# Finch

![Score 76/100 — Grade B+](../../web/badge/developer.tryfinch.com.svg)

Category: **data** · [Live llms.txt](https://developer.tryfinch.com/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** Finch


**File facts:** 21.4 KB · 3 `## sections` · 110 links (110 after low-value discount) · Freshness: 0 days (per `Last-Modified`).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 16/18 | _no_blockquote_summary_ |
| Coverage | 12/20 | |
| Agent-action declarations | 10/14 | _no_llms_full_link_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 10/10 | |
| Discoverability | 8/8 | |
| Auth signposting | 3/8 | _no_auth_section_ |
| Size discipline | 5/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Freshness (10/10)
- Discoverability (8/8)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Auth signposting (3/8): no_auth_section

## Embed the badge

```markdown
[![llms.txt score 76 (B+)](https://raw.githubusercontent.com/agentrhq/awesome-llms-txt/main/web/badge/developer.tryfinch.com.svg)](https://github.com/agentrhq/awesome-llms-txt/tree/main/sites/developer.tryfinch.com)
```

## Reproduce this score

```bash
npx llms-txt-score https://developer.tryfinch.com/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.

---

Curated by [Authsome](https://authsome.dev) · agent identity for third-party APIs.
