---
domain: www.gu.ma
display_name: Guma - Wellness Programs Platform
category: dev-tools
score: 50
grade: C
last_scored: 2026-05-27
verified_url: https://www.gu.ma/llms.txt
file_size_bytes: 917
link_count: 0
value_link_count: 0
section_count: 3
freshness_age_days: 0
---

# Guma - Wellness Programs Platform

![Score 50/100 — Grade C](../../web/badge/www.gu.ma.svg)

Category: **dev-tools** · [Live llms.txt](https://www.gu.ma/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** Guma - Wellness Programs Platform

> Guma is a wellness platform connecting people with qualified practitioners through personalized programs.

**File facts:** 0.9 KB · 3 `## sections` · 0 links (0 after low-value discount) · Freshness: 0 days (per `Last-Modified`).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 11/18 | _sections_without_link_lists, malformed_link_items_ |
| Coverage | 3/20 | _few_value_links, missing_canonical_sections_ |
| Agent-action declarations | 0/14 | _no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 10/10 | |
| Discoverability | 8/8 | |
| Auth signposting | 0/8 | _no_auth_keywords, no_auth_section_ |
| Size discipline | 6/6 | |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Freshness (10/10)
- Discoverability (8/8)
- Size discipline (6/6)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Coverage (3/20): few_value_links, missing_canonical_sections
- Agent-action declarations (0/14): no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec
- Auth signposting (0/8): no_auth_keywords, no_auth_section

## Embed the badge

```markdown
[![llms.txt score 50 (C)](https://raw.githubusercontent.com/agentrhq/awesome-llms-txt/main/web/badge/www.gu.ma.svg)](https://github.com/agentrhq/awesome-llms-txt/tree/main/sites/www.gu.ma)
```

## Reproduce this score

```bash
npx llms-txt-score https://www.gu.ma/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.

---

Curated by [Authsome](https://authsome.dev) · agent identity for third-party APIs.
