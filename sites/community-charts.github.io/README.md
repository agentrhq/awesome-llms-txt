---
domain: community-charts.github.io
display_name: Community Charts
category: infra
score: 73
grade: B
last_scored: 2026-05-27
verified_url: https://community-charts.github.io/llms.txt
file_size_bytes: 79998
link_count: 258
value_link_count: 60
section_count: 3
freshness_age_days: 6
---

# Community Charts

![Score 73/100 — Grade B](../../web/badge/community-charts.github.io.svg)

Category: **infra** · [Live llms.txt](https://community-charts.github.io/llms.txt) · Snapshot: [`llms.txt`](./llms.txt) · Machine-readable: [`score.json`](./score.json)

**H1:** Community Charts

> Discover community-driven Helm charts for open-source tools lacking official support. Deploy apps effortlessly on Kubernetes.

**File facts:** 78.1 KB · 3 `## sections` · 258 links (60 after low-value discount) · Freshness: 6 days (per `Last-Modified`).

## Scorecard

| Criterion | Score | Notes |
|---|---:|---|
| Spec compliance | 18/18 | |
| Coverage | 14/20 | _mostly_low_value_198_of_258_ |
| Agent-action declarations | 5/14 | _no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec_ |
| Linked-content stability | 6/10 | _not_sampled_ |
| Freshness | 10/10 | |
| Discoverability | 8/8 | |
| Auth signposting | 4/8 | |
| Size discipline | 2/6 | _oversized_78kb_ |
| Content-Type & encoding | 4/4 | |
| Voice | 2/2 | |

## What's exceptional

- Spec compliance (18/18)
- Freshness (10/10)
- Discoverability (8/8)
- Content-Type & encoding (4/4)
- Voice (2/2)

## What's weak

- Agent-action declarations (5/14): no_llms_full_link, no_agent_signposts, no_machine_readable_api_spec
- Size discipline (2/6): oversized_78kb

## Embed the badge

```markdown
[![llms.txt score 73 (B)](https://raw.githubusercontent.com/agentrhq/awesome-llms-txt/main/web/badge/community-charts.github.io.svg)](https://github.com/agentrhq/awesome-llms-txt/tree/main/sites/community-charts.github.io)
```

## Reproduce this score

```bash
npx llms-txt-score https://community-charts.github.io/llms.txt
```

See [the rubric](../../RUBRIC.md) for what each criterion checks.
